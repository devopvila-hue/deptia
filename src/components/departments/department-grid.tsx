...
export function DepartmentGrid({ department }: { department: Department }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-start justify-between border-b border-border/60 pb-4">
        ...
        >
          {department.category} · 0{department.ordering}
        </span>
      ...
      <div className="mt-6 grid grid-cols-3 gap-2">
        {department.metrics.map((m) => (
          <div key={m.label} className="rounded-md border border-border/60 bg-surface-soft/40 p-2.5">
      ...
      <div className="mt-auto flex items-center justify-between border-t border-border/60 pt-5">
        <div>
          <p className="font-mono text-[0.6rem] uppercase tracking-[0.14em] text-muted">Desde</p>
          <p className="mt-0.5 font-display text-[1.25rem] tracking-[-0.02em] text-foreground">
            {department.priceFrom}€ <span className="text-[0.875rem] text-muted">/ mes</span>
          </p>
        </div>
        <Link
